...
typedef struct _Clay_SDL3RendererData {
...
static void SDL_Clay_RenderFillRoundedRect(
    ...
    vertices[vertexCount++] = (SDL_Vertex){
        {rect.x + clampedRadius, rect.y + clampedRadius}, color, {0, 0} };                      //0 center TL
    vertices[vertexCount++] = (SDL_Vertex){
        {rect.x + rect.w - clampedRadius, rect.y + clampedRadius}, color, {1, 0} };             //1 center TR
    vertices[vertexCount++] = (SDL_Vertex){
        {rect.x + rect.w - clampedRadius, rect.y + rect.h - clampedRadius}, color, {1, 1} };    //2 center BR
    vertices[vertexCount++] = (SDL_Vertex){
        {rect.x + clampedRadius, rect.y + rect.h - clampedRadius}, color, {0, 1} };             //3 center BL
        ...
        switch (rcmd->commandType) {
            ...
            case CLAY_RENDER_COMMAND_TYPE_BORDER: {
                ...
                if (config->cornerRadius.bottomRight > 0) {
                    ...
                    const float centerX = rect.x + rect.w - clampedRadii.bottomRight -1;
